...
let search_box = document.getElementById('input_box');
search_box.addEventListener('keypress', (event) => {
    if (event.keyCode == 13) {
        
        getWeatherReport(search_box.value);
...
function showWeaterReport(weather) {
    ...
    let op = document.getElementById('weather_website_str');
    op.style.display = 'block';
    let todayDate = new Date();
    let parent=document.getElementById('parent');
    let weather_body = document.getElementById('weather_website_str');
